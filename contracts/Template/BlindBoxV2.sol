...
contract BlindBoxV2 is ERC721Enumerable, InitializableOwnable, Pausable {
    ...
    uint256[] public per721Supplys;
    uint256[] public per1155Supplys;
    address[] public erc20Tokens;

    // uint256[] public serverIds;

    mapping (string=>mapping (uint256=>uint256)) serverIds;
    ...
    // mapping (address=>EnumerableSet.UintSet) private pools;
    ...
    struct boxState {
        bool isOpen;
        EnumerableSet.UintSet realTokenId;
    }
    ...
    event Open(address owner, uint256 boxId, uint256 tokenId, uint256 serverId);
    ...
    function initialize(
        ...
        for (uint256 i = 0; i < _per721Supplys.length; i++) {
            per721Supplys.push(_per721Supplys[i]);
            serverIds["ERC721"][i] = _serverIds[i];
        }

        uint256 startAt = _per721Supplys.length;
        for (uint256 i = 0; i < _per1155Supplys.length; i++) {
            per1155Supplys.push(_per1155Supplys[i]);
            serverIds["ERC1155"][i] = _serverIds[startAt + i];
        }

        startAt = startAt + _per1155Supplys.length;
        for (uint256 i = 0; i < _erc20Tokens.length; i++) {
            erc20Tokens[i] = _erc20Tokens[i];
            tokensSupply[_erc20Tokens[i]] = _tokensSupply[i];
            IERC20(erc20Tokens[i]).transferFrom(_msgSender(), address(this), _tokensSupply[i] * maxSupply);
            serverIds["ERC20"][i] = _serverIds[startAt + i];
        }

        for (uint256 i = 3; i < _addressConfigs.length; i++) {
            salePrices[_addressConfigs[i]] = _saleSettings[i+1];
        }
    ...
    function mint(uint256 count, address token, uint8 v, bytes32 r, bytes32 s) external payable onlyEOA {
        bytes32 messageHash = keccak256(abi.encodePacked(this, _msgSender(), count, token));
        ...
        if (token == address(0)) {
            require(salePrices[token] * count == msg.value, "BlindBox: incorrect Ether value");
            // The contract never holds any Ether. Everything gets redirected to treasury directly.
            Address.sendValue(payable(_saleConfig.treasury), msg.value);
            // payable(_saleConfig.treasury).call{value: msg.value}("");
        }else{
            uint256 amount = salePrices[token] * count;
            IERC20(_saleConfig.saleToken).transferFrom(_msgSender(), _saleConfig.treasury, amount);
        }
    ...
    function open(uint256 _boxTokenId, uint8 v, bytes32 r, bytes32 s) external onlyEOA {
        ...
        for (uint256 i = 0; i < per721Supplys.length; i++) {
            uint256 itemCount = per721Supplys[i] / maxSupply;
            for (uint256 k = 0; k < itemCount; k++) {
                uint256 tokenId = totalSupply() + 1;

                _safeMint(_msgSender(), tokenId);
                // pools[j].add( _nextTokenId + ind + j + k);
                boxStatus[_boxTokenId].realTokenId.add(tokenId);

                emit Open(msg.sender, _boxTokenId, tokenId, serverIds["ERC721"][i]);
            }
        }

        uint256 length = per1155Supplys.length;
        for (uint256 i = 0; i < length; i++) {
            uint256 itemCount = per1155Supplys[i] / maxSupply;
            for (uint256 k = 0; k < itemCount; k++) {
                uint256 tokenId = totalSupply() + 1;
                ...
                emit Open(msg.sender, _boxTokenId, tokenId, serverIds["ERC1155"][i]);
            }
        }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        if(boxStatus[tokenId].isOpen == true){
            tokenId = boxStatus[tokenId].realTokenId.at(tokenId);
